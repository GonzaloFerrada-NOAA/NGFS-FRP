...
for d in dates:
    ...
    file_in = f"{path_in}/NGFS_FIRE_DETECTIONS_{satellite}_ABI_CONUS_{sdate}_{sdoy}.csv"
    checkFile(file_in)
    
    try:
        # Read the wildfire data from the CSV file
        msg(f"Reading data from {file_in}")
        ...
        # Save
        if save_csv:
            file_out = f"{path_csv}/ngfs_gridded_{R}_{satellite}_{sdate}.csv"
